...
var rule = {
  ...
  homeUrl: "",
  url: "https://www.jiabaide.cn/api/mw-movie/anonymous/video/list?pageNum=fypage&pageSize=30&sort=1&sortBy=1&type1=fyclass",
  searchUrl:
    "/api/mw-movie/anonymous/video/searchByWordPageable?keyword=**&pageNum=fypage&pageSize=12&type=false",
  searchable: 1,
  quickSearch: 1,
  timeout: 5000,
  play_parse: true,
  ...
  一级: async function (tid, pg, filter, extend) {
    ...
    const signkey =
      `pageNum=${pg}&pageSize=30&sort=1&sortBy=1&type1=${tid}&key=cb808529bae6b6be45ecfab29a4889bc&t=` +
      t;
    const key = CryptoJS.SHA1(CryptoJS.MD5(signkey).toString()).toString();
    const html = JSON.parse(
      (
        await req(
          `https://m.jiabaide.cn/api/mw-movie/anonymous/video/list?pageNum=${pg}&pageSize=30&sort=1&sortBy=1&type1=${tid}`,
          {
            headers: {
              "User-Agent":
  ...
  二级: async function (ids) {
    ...
    const key = CryptoJS.SHA1(CryptoJS.MD5(signkey).toString()).toString();
    const html = await req_(
      `https://www.jiabaide.cn/api/mw-movie/anonymous/video/detail?id=${id_}`,
      "get",
      {
        "User-Agent":
          "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/127.0.0.0 Safari/537.36",
  ...
  搜索: async function (wd, quick, pg) {
    ...
    const signkey =
      ...
      "&pageNum=" +
      pg +
      "&pageSize=12&type=false&key=cb808529bae6b6be45ecfab29a4889bc&t=" +
      t;
    const key = CryptoJS.SHA1(CryptoJS.MD5(signkey).toString()).toString();
    let html = JSON.parse(
      (
  ...
  lazy: async function (flag, id, flags) {
    ...
    const t = new Date().getTime();
    const signkey =
      `clientType=1&id=${pid}&nid=${nid}&key=cb808529bae6b6be45ecfab29a4889bc&t=` +
      t;
    const key = CryptoJS.SHA1(CryptoJS.MD5(signkey).toString()).toString();
    const relurl =
      "https://m.jiabaide.cn/api/mw-movie/anonymous/v2/video/episode/url?clientType=1&id=" +
      pid +
      "&nid=" +
      nid;
