globalThis.h_ost = 'https://api.xingzhige.com';
...
var rule = {
    ...
    一级: async function() {
        ...
        const url = `${h_ost}/API/playlet/?keyword=${encodeURIComponent(cid)}&page=${page}`;
        try {
    ...
    二级: async function() {
        ...
        try {
               book_id = book_id || orId.split('@')[5];
                const detailUrl = `${h_ost}/API/playlet/?book_id=${book_id}`;
    ...
    搜索: async function() {
        ...
        const url = `${h_ost}/API/playlet/?keyword=${encodeURIComponent(KEY)}&page=${page}`;
        const videos = [];
        try {
            const response = await request(url, {headers: rule.headers});
            const data = JSON.parse(response).data;
    ...
    lazy: async function() {
        const {input} = this;
        const url = `${h_ost}/API/playlet/?video_id=${input}&quality=1080p`;
